...
> 📊 **그림 1**: Git 호스팅 플랫폼별 핵심 포지셔닝

```mermaid
graph TD
    G["Git 호스팅 플랫폼"] --> GH["GitHub<br/>커뮤니티 + AI"]
    G --> GL["GitLab<br/>올인원 DevOps"]
    G --> BB["Bitbucket<br/>Atlassian 연동"]
    GH --> GH1["1억+ 개발자"]
    GH --> GH2["Copilot"]
    GH --> GH3["Actions CI/CD"]
    GL --> GL1["셀프 호스팅"]
    GL --> GL2["내장 CI/CD"]
    GL --> GL3["보안 스캔"]
    BB --> BB1["Jira 통합"]
    BB --> BB2["Confluence 연동"]
    BB --> BB3["Pipelines"]
```
...
> 📊 **그림 2**: GitOps 워크플로우 — Git 저장소 중심의 선언적 배포

```mermaid
flowchart LR
    DEV["개발자"] -->|커밋/PR| REPO["Git 저장소<br/>선언적 설정"]
    REPO -->|감지| ARGO["ArgoCD / Flux"]
    ARGO -->|비교| K8S["Kubernetes 클러스터"]
    K8S -->|상태 보고| ARGO
    ARGO -->|차이 발견시<br/>자동 동기화| K8S
    K8S --> APP["운영 중인 앱"]
```
...
> 📊 **그림 4**: Git 학습 로드맵 — 이 튜토리얼 이후의 단계

```mermaid
flowchart TD
    NOW["현재: 튜토리얼 완료"] --> A["즉시 실천"]
    NOW --> B["단기 1-3개월"]
    NOW --> C["중장기 3-6개월"]
    A --> A1["개인 프로젝트 적용"]
    A --> A2["GitHub 프로필 정리"]
    A --> A3["첫 오픈소스 PR"]
    B --> B1["GitHub Actions CI/CD"]
    B --> B2["코드 리뷰 실천"]
    B --> B3["Git 내부 구조 학습"]
    C --> C1["GitOps - ArgoCD/Flux"]
    C --> C2["AI 도구 활용"]
    C --> C3["모노레포/서브모듈"]
```
