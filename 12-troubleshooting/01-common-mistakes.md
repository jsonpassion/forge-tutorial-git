...
> 📊 **그림 1**: Git 실수 유형별 복구 전략 선택 흐름

```mermaid
flowchart TD
    A["실수 발생!"] --> B{"이미 push 했나?"}
    B -- 아니오 --> C{"어떤 실수?"}
    B -- 예 --> D{"공유 브랜치인가?"}
    C --> C1["메시지 오타<br/>amend"]
    C --> C2["파일 빠뜨림<br/>add + amend"]
    C --> C3["잘못된 브랜치<br/>branch + reset"]
    D -- 개인 브랜치 --> E["reset + force-with-lease"]
    D -- 공유 브랜치 --> F["revert 사용"]
...
> 📊 **그림 2**: 잘못된 브랜치 커밋 복구 과정 (push 전)

```mermaid
sequenceDiagram
    participant M as main
    participant F as feature 브랜치
    Note over M: 잘못된 커밋이 main에 있음
    M->>F: 1. git branch feature<br/>(커밋 복사)
    Note over M: 2. git reset --hard HEAD~1<br/>(main에서 제거)
    Note over F: 3. git switch feature<br/>(올바른 브랜치로 이동)
    Note over M: main 원래 상태 복원
    Note over F: 커밋이 여기에 존재
```
...
> 📊 **그림 3**: push 취소 — force push vs revert 비교

```mermaid
flowchart LR
    subgraph 개인브랜치["개인 브랜치"]
        A1["reset --hard"] --> A2["force-with-lease push"]
        A2 --> A3["히스토리에서 제거됨"]
    end
    subgraph 공유브랜치["공유 브랜치 - main"]
        B1["git revert"] --> B2["새 되돌리기 커밋 생성"]
        B2 --> B3["git push"]
        B3 --> B4["히스토리 보존, 안전"]
    end
...
> 📊 **그림 4**: Git 복구 명령어의 안전도 스펙트럼

```mermaid
graph LR
    A["안전"] -.-> B["주의 필요"] -.-> C["위험"]
    A1["revert<br/>히스토리 보존"] --> A
    A2["reflog + checkout -b<br/>브랜치 복구"] --> A
    B1["amend<br/>push 전만 사용"] --> B
    B2["force-with-lease<br/>개인 브랜치만"] --> B
    C1["reset --hard<br/>미커밋 변경 소실"] --> C
    C2["force push<br/>타인 커밋 덮어쓰기"] --> C
