...
> 📊 **그림 1**: CLI와 GUI의 최적 사용 영역

```mermaid
flowchart LR
    subgraph CLI["CLI 영역"]
        A1["자동화<br/>스크립트, CI/CD"]
        A2["원격 서버<br/>SSH 작업"]
        A3["고급 명령<br/>rebase, filter-repo"]
        A4["반복 작업<br/>alias 활용"]
    end
    subgraph BOTH["혼용 영역"]
        B1["커밋, 푸시"]
        B2["브랜치 관리"]
...
> 📊 **그림 2**: Git GUI 도구 선택 의사결정 흐름

```mermaid
flowchart TD
    Q1{"이미 VS Code를<br/>사용 중인가?"}
    Q1 -- 예 --> R1["GitLens + Git Graph<br/>확장 설치"]
    Q1 -- 아니오 --> Q2{"터미널을 벗어나고<br/>싶지 않은가?"}
    Q2 -- 예 --> R2["lazygit<br/>TUI"]
    Q2 -- 아니오 --> Q3{"시각적 커밋 그래프가<br/>중요한가?"}
    Q3 -- 예 --> R3["GitKraken"]
    Q3 -- 아니오 --> Q4{"가볍고 빠른<br/>전용 앱?"}
    Q4 -- 예 --> R4["Fork<br/>macOS/Windows"]
    Q4 -- 아니오 --> Q5{"GitHub 중심<br/>워크플로?"}
    Q5 -- 예 --> R5["GitHub Desktop"]
    Q5 -- 아니오 --> R6["Sourcetree<br/>Jira/Bitbucket 연동"]
```
...
> 📊 **그림 3**: Git GUI 도구 생태계의 발전

```mermaid
graph LR
    A["2005<br/>Git 탄생<br/>CLI만 존재"] --> B["2006<br/>gitk, git-gui<br/>번들 포함"]
    B --> C["2011<br/>Sourcetree<br/>Atlassian"]
    C --> D["2015<br/>VS Code 출시<br/>내장 Git"]
    D --> E["2016<br/>GitKraken<br/>시각적 전문 도구"]
    E --> F["2017<br/>Fork<br/>네이티브 클라이언트"]
    F --> G["현재<br/>에디터 + GUI + CLI<br/>조합이 표준"]
```
