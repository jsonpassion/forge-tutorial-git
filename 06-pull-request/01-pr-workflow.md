...
> 📊 **그림 2**: 혼자 개발 vs 팀 협업 — PR이 필요한 이유

```mermaid
flowchart TD
    subgraph solo["혼자 개발"]
        S1["코드 작성"] --> S2["main에 바로 커밋"]
    end
    subgraph team["팀 협업"]
        T1["코드 작성"] --> T2["PR 생성"]
        T2 --> T3["코드 리뷰"]
...
> 📊 **그림 1**: GitHub Flow — PR 중심 협업 워크플로우의 6단계

```mermaid
flowchart LR
    A["1. 브랜치 생성"] --> B["2. 작업 & 커밋"]
    B --> C["3. PR 생성"]
    C --> D["4. 코드 리뷰"]
    D --> E["5. 머지"]
    E --> F["6. 브랜치 삭제"]
...
> 📊 **그림 4**: CODEOWNERS — 파일별 자동 리뷰어 배정 구조

```mermaid
flowchart TD
    PR["PR 생성"] --> CHECK{"변경된 파일 확인"}
    CHECK --> FE["/src/frontend/*"]
    CHECK --> AUTH["/src/auth/*"]
    CHECK --> DOCS["*.md"]
    CHECK --> OTHER["기타 파일"]
    FE --> R1["@frontend-team"]
    AUTH --> R2["@security-team<br/>@team-lead"]
    DOCS --> R3["@tech-writer"]
    OTHER --> R4["@team-lead"]
```
...
stateDiagram-v2
    ...
    Open --> ReviewInProgress : 리뷰어 배정
    ReviewInProgress --> ChangesRequested : 수정 요청
    ChangesRequested --> ReviewInProgress : 수정 후 재리뷰
    ReviewInProgress --> Approved : 승인
    Approved --> Merged : 머지
    Open --> Closed : 닫기
    Merged --> [*]
    Closed --> [*]
