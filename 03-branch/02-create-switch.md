...
> 📊 **그림 1**: git branch는 포인터만 생성하고 HEAD는 그대로
...
graph LR
    ...
    HEAD["HEAD"] -.->|"현재 위치"| main
    style HEAD fill:#f9a825,color:#000
    style feature fill:#66bb6a,color:#000
    style main fill:#42a5f5,color:#000
```
...
> 📊 **그림 2**: git checkout의 역할 분리 (Git 2.23)

```mermaid
flowchart TD
    A["git checkout<br/>Git 초기 ~ 2019"] -->|"브랜치 전환 역할"| B["git switch<br/>Git 2.23+"]
    A -->|"파일 복원 역할"| C["git restore<br/>Git 2.23+"]
    A -.->|"여전히 사용 가능<br/>레거시"| D["git checkout<br/>두 역할 모두"]
    style A fill:#ef5350,color:#fff
    style B fill:#42a5f5,color:#fff
    style C fill:#66bb6a,color:#fff
    style D fill:#bdbdbd,color:#000
```
...
> 📊 **그림 3**: 브랜치 전환 시 Git의 내부 동작 3단계

```mermaid
flowchart LR
    A["git switch"] --> B["1. HEAD 업데이트<br/>.git/HEAD 변경"]
    B --> C["2. 인덱스 갱신<br/>스테이징 영역 업데이트"]
    C --> D["3. 작업 디렉토리<br/>파일 상태 변경"]
...
> 📊 **그림 4**: 미커밋 변경 사항이 있을 때 브랜치 전환 흐름

```mermaid
flowchart TD
    A["git switch 다른브랜치"] --> B{"미커밋 변경 사항<br/>충돌 여부?"}
    B -->|"충돌 없음"| C["변경 사항 carry over<br/>전환 성공"]
    B -->|"충돌 있음"| D["전환 거부<br/>error 메시지"]
    D --> E["git commit<br/>커밋하기"]
    D --> F["git stash<br/>임시 저장"]
    D --> G["git restore .<br/>변경 사항 버리기"]
    E --> H["전환 재시도"]
    F --> H
    G --> H
