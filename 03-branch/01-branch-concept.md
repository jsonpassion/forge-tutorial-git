...
> 📊 **그림 4**: 브랜치와 태그의 동작 차이 — 새 커밋 시 브랜치만 이동

```mermaid
flowchart LR
    subgraph before["커밋 전"]
        T1["v1.0 태그"] --> CA["커밋 A"]
        B1["main 브랜치"] --> CA
    end
    subgraph after["커밋 후"]
        T2["v1.0 태그"] --> CB["커밋 A"]
        B2["main 브랜치"] --> CC["커밋 B(새 커밋)"]
        CC --> CB
    end
    before -.->|"git commit"| after
```
...
> 📊 **그림 1**: HEAD → 브랜치 → 커밋의 포인터 체인 구조

```mermaid
flowchart LR
    HEAD[".git/HEAD"] -->|"ref: refs/heads/main"| B["main 브랜치"]
    B -->|"a1b2c3d..."| C["커밋 객체"]
    C --> D["트리 객체"]
    D --> E["파일 스냅샷"]
```
...
> 📊 **그림 2**: 정상 HEAD vs Detached HEAD 상태 비교
...
flowchart TD
    ...
    subgraph 분리["Detached HEAD 상태"]
        H2["HEAD"] -->|"직접 가리킴"| C2a["커밋 C2"]
        M2["main"] --> C3a["커밋 C3"]
        C3a --> C2a
    end
```
...
> 📊 **그림 3**: 브랜치가 갈라진 커밋 그래프(DAG) 구조

```mermaid
gitGraph
    commit id: "a1b2c3d" tag: "초기 커밋"
    branch feature
    checkout main
    commit id: "b2c3d4e" tag: "버그 수정"
    checkout feature
    commit id: "c3d4e5f" tag: "로그인 폼"
    commit id: "d4e5f6g" tag: "소셜 로그인"
```
