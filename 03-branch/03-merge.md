...
> 📊 **그림 1**: Fast-Forward 머지 — main 포인터가 feature 위치로 이동
...
> 📊 **그림 2**: 3-Way 머지 — 공통 조상을 기준으로 양쪽 변경 사항을 합침

```mermaid
flowchart TD
    A["공통 조상<br/>Common Ancestor"] --> B["main 최신 커밋<br/>ours"]
    A --> C["feature 최신 커밋<br/>theirs"]
    B --> D["머지 커밋<br/>Merge Commit"]
    C --> D
...
flowchart LR
    ...
    subgraph NOFF["--no-ff"]
        direction TB
        A2["A"] --> M2["Merge Commit"]
        A2 --> B2["B"] --> C2["C"] --> M2
    end
    subgraph THREE["3-Way Merge"]
        direction TB
        A3["A"] --> B3["main 커밋"]
        A3 --> C3["feature 커밋"]
        B3 --> M3["Merge Commit"]
...
stateDiagram-v2
    ...
    머지시작 --> 충돌발생: 충돌 있음
    충돌발생 --> 충돌해결중: 파일 수정
    충돌해결중 --> 머지완료: git commit
    충돌발생 --> 취소: git merge --abort
    충돌해결중 --> 취소: git merge --abort
    자동완료 --> 되돌리기: git revert -m 1
    머지완료 --> 되돌리기: git revert -m 1
    취소 --> [*]
    되돌리기 --> [*]
```
