...
> 📊 **그림 1**: 충돌 발생 원리 — 같은 파일의 같은 부분을 두 브랜치에서 수정

```mermaid
flowchart TD
    A["공통 조상 커밋<br/>'저는 개발자입니다'"] --> B["main 브랜치<br/>'저는 백엔드 개발자입니다'"] 
    A --> C["feature 브랜치<br/>'저는 프론트엔드 개발자입니다'"]
    B --> D{"merge 시도"}
    C --> D
    D --> E["충돌 발생!<br/>Git이 자동 판단 불가"]
```
...
```mermaid
flowchart TD
    A["<<<<<<< HEAD"] --> B["현재 브랜치의 내용<br/>ours"]
    B --> C["======="]
    C --> D["병합 브랜치의 내용<br/>theirs"]
    D --> E[">>>>>>> feature"]
...
> 📊 **그림 4**: 기본 마커 vs diff3 마커 비교

```mermaid
flowchart LR
    subgraph 기본["기본 스타일"]
        direction TB
        X1["HEAD 내용"] --- X2["======="] --- X3["feature 내용"]
    end
    subgraph D3["diff3 스타일"]
        direction TB
        Y1["HEAD 내용"] --- Y2["원본 내용"] --- Y3["======="] --- Y4["feature 내용"]
    end
    기본 -- "원본이 보여서<br/>의도 파악이 쉬움" --> D3
```
...
```mermaid
flowchart LR
    A["충돌 발생"] --> B["git status로<br/>충돌 파일 확인"]
    B --> C["파일 열어서<br/>마커 확인"]
    C --> D{"해결 방법 선택"}
    D --> E["ours 선택"]
    D --> F["theirs 선택"]
    D --> G["양쪽 합치기"]
    E --> H["마커 제거 후<br/>내용 작성"]
    F --> H
    G --> H
    H --> I["git add"]
    I --> J["git commit"]
