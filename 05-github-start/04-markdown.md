...
> 📊 **그림 2**: README 방문자의 관심 흐름 — 5초 안에 답을 줘야 합니다

```mermaid
flowchart TD
    A["방문자 도착"] --> B{"이게 뭐지?"}
    B -->|"5초 이내"| C["프로젝트 소개<br/>한 줄 설명 + 배지"]
    C --> D{"어떻게 쓰지?"}
    D -->|"1분 이내"| E["빠른 시작<br/>설치 + 기본 사용법"]
    E --> F{"더 알고 싶다"}
    F -->|"필요할 때"| G["상세 정보<br/>설정, API, 아키텍처"]
    G --> H["부가 정보<br/>라이선스, 기여 가이드"]
    B -->|"답 없음"| I["이탈"]
```
