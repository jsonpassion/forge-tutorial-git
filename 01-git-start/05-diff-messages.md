...
> 📊 **그림 1**: git diff가 비교하는 세 영역

```mermaid
flowchart LR
    WD["작업 디렉토리<br/>Working Directory"] -->|"git diff"| ST["스테이징 영역<br/>Staging Area"]
    ST -->|"git diff --staged"| LC["최근 커밋<br/>Last Commit"]
    WD -->|"git diff HEAD"| LC
```
...
> 📊 **그림 3**: Conventional Commits 메시지 형식

```mermaid
flowchart LR
    A["타입<br/>feat, fix, docs..."] --> B["범위 - 선택"] --> C["설명<br/>50자 이내 요약"]
    C --> D{"본문 필요?"}
    D -->|"예"| E["빈 줄 + 상세 설명"]
    D -->|"아니오"| F["커밋 완료"]
    E --> G["꼬리말<br/>Resolves: #42"]
    G --> F
```
