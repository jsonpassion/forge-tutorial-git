...
> 📊 **그림 2**: 매트릭스 빌드 — 3 OS x 3 Node 조합

```mermaid
graph TD
    T["매트릭스 빌드 트리거"] --> U["Ubuntu"]
    T --> M["macOS"]
    T --> W["Windows"]
    U --> U18["Node 18"]
    U --> U20["Node 20"]
...
> 📊 **그림 3**: 캐시 동작 흐름 — key 매칭 과정

```mermaid
flowchart TD
    A["캐시 복원 시도"] --> B{"key 정확히 일치?"}
    B -- 예 --> C["캐시 적중(hit)<br/>설치 건너뜀"]
    B -- 아니오 --> D{"restore-keys<br/>부분 일치?"}
    D -- 예 --> E["부분 캐시 복원<br/>+ 추가 설치"]
    D -- 아니오 --> F["캐시 미스(miss)<br/>전체 새로 설치"]
    E --> G["Job 완료 후<br/>캐시 저장"]
    F --> G
```
...
sequenceDiagram
    ...
    CI->>CI: 빌드 + 테스트 실행
    alt 성공
        CI->>GH: Status Check 통과
        GH->>D: 머지 가능 표시
    else 실패
        CI->>GH: Status Check 실패
        GH->>D: 머지 차단 + 알림
    end
```
